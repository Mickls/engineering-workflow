...
工作流用项目上下文、真实场景驱动的单题澄清、需求约束、风险触发的 AFK/HITL 切片、编码前验收场景、实现路径契约、完整链路测试和基于证据的交付门禁，减少疲劳确认、“测试全过但实际不可用”、实现偏离和不必要的人工 review。
...
非轻量需求先创建 `README.md(status=clarifying)` 和 `clarification.md`。AI 查证项目事实，为候选问题证明真实场景和决定必要性，维护问题地图并一次解释一个；用户只确认行为、取舍和风险，项目已有模式覆盖的技术细节由 AI 负责。用户明确批准澄清核销总结后，才创建权威 `design.md` 和 `plan.md`。

实现计划按用户可观察闭环标记 AFK/HITL。验收场景在编码前确定；已批准、符合项目模式且可从真实入口验证的 AFK 切片可以连续执行，只有新决定、方案偏离、证据缺口、手动步骤、未覆盖风险或用户 review budget 命中时暂停。HITL 和最终交付先提供普通语言的行为/风险 review 包，再附技术证据。已完成历史 issue 不追溯补建；重新打开或发生重大需求变化时应用新规则。
