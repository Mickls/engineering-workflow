...
## 3. Engineering Workflow Package

工程化规则由 `engineering-workflow` package 统一管理。`AGENTS.md` 只保留常驻原则和路由，避免每次会话注入大段细则。
...
| 长任务交接、切换会话、交给另一个 agent 继续项目工作 | `engineering-workflow:engineering-handoff` |
| lint/test/build、完成声明、提交前检查、最终交付格式 | `engineering-workflow:verification-delivery` |

使用规则：

- 非轻量编码任务开始前，至少检查是否需要 `requirements-workflow`、`coding-standards`。
- 首次进入复杂项目或发现项目命令、术语、issue 约定缺失时，应使用 `project-setup` 建立项目上下文。
- 普通 bug 和失败测试先用 `diagnosis-workflow` 建立反馈循环；线上事故、数据误删、幂等、并发和重复调用问题使用 `incident-debugging`。
- 线上问题、生产 bugfix、数据误删、重复调用、幂等性或并发问题，必须使用 `incident-debugging`；未从真实入口复现并证明根因前，不得修改生产逻辑或宣称已修复。
...
- 完成任何代码或文档修改前，必须使用 `verification-delivery`。
...
- 不清楚且会影响实现方案的问题必须先和用户确认；能通过代码、文档或测试确认的问题不要直接追问。
...
- 需要编码的非轻量需求默认按 `requirements-workflow` 创建或维护 `issue/` 文档。
- 非轻量需求必须把 design 中的关键约束、边界条件、验收标准提炼为可追踪覆盖表，并在 plan 中逐条映射到实施步骤、代码落点和测试/验证入口。
- 非轻量需求创建或更新 `design.md` / `plan.md` 后，必须停止当前回合并请用户 review；没有后续明确确认前，不得进入编码、测试编写、migration/proto 生成或实现类子任务。
- 轻量任务可以跳过 issue 文档，但最终回复必须说明轻量判断、修改范围和验证方式。

## 5. 编码

- 禁止重复造轮子：优先项目已有能力，其次 Go 标准库，其次项目已引入第三方库，最后才自己实现。
- 数据校验默认只放在调用链顶层；内部函数不要重复做防御式校验，除非它本身就是外部边界或确有未校验路径能到达。
- 减少临时中间类型；只有跨模块、跨 goroutine、序列化、稳定接口或明显提升可读性时才定义新类型。
- 错误处理、日志、DAO/repository、migration、依赖管理按 `coding-standards` 执行。
- 实现非轻量需求时，必须回填关键约束覆盖矩阵；每个约束要有实现落点、验证证据或明确的不适用/阻塞理由。
...
- 纯格式、注释、文案、无行为变化的轻量整理可以不新增测试，但必须说明原因。
- 测试失败后，不允许立刻按失败表象修改逻辑代码；先按 `testing-policy` 调查调用链、重构影响和测试本身是否仍能证明目标行为。
- 只有确认失败暴露真实逻辑错误时，才考虑修改逻辑代码；若该逻辑修改不在已确认计划内，需先向用户说明并得到允许。

## 7. 子 Agent

- 本文件表达用户对工程任务默认使用 Codex 原生子代理能力的长期偏好：除非用户在当前任务中明确禁止，主线程应优先考虑通过子 agent 做并行调查、分工实现或交叉 review。
- 子 agent 的创建、等待、关闭、恢复和上下文管理属于 Codex 运行时能力，必须遵循当前工具说明和系统限制；工程化 skills 不得定义或覆盖这些能力。
- 如果当前工具规则仍要求本轮显式授权，以工具规则为准，并向用户说明无法默认开启的原因。
- 如果任务轻量、强耦合、写入范围冲突或子 agent 工具不可用，可以不使用子 agent，但需要在计划或交付中说明原因。
- 实际创建子 agent 时，主线程必须维护项目空间内的会话级运行期登记表。默认路径为 `<workspace>/.codex/${CODEX_THREAD_ID}/subagents.md`；如果当前运行时没有 `CODEX_THREAD_ID`，则使用运行时提供的 thread/session id 作为目录名，并在创建第一个子 agent 前明确本会话登记表路径。
- 子 agent 登记表只记录当前项目当前会话的子代理状态，不属于生产代码或项目交付产物，默认不提交。
- 每次创建子 agent 后，必须立刻记录工具返回的 agent id、昵称或角色、任务、创建时间和状态 `running`。
- 每次 `wait`、收到完成结果、失败、取消或关闭子 agent 后，必须同步更新登记表状态，例如 `completed`、`failed`、`cancelled`、`closed` 或 `unknown`。
- 上下文压缩、恢复对话、准备创建新子 agent、最终交付前，都应先检查本会话登记表；对仍为 `running`/`completed` 但已不再需要的子 agent，按当前工具能力关闭并更新为 `closed`。
- 如果当前工具不提供关闭能力、登记表缺失、agent id 无效或关闭失败，必须在最终回复中说明真实状态和剩余风险，不得声称已关闭。
