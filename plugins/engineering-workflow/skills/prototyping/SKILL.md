...
- 默认写入 `.codex/engineering-workflow/prototypes/`；如果原型绑定某个需求，写入 `.codex/engineering-workflow/issues/<REQ-or-EPIC>/`。
- 默认不持久化；如必须持久化，使用 scratch DB 或清晰标记的临时文件。
- 只做足以帮助用户判断的有限自检。
- 原型阶段不触发生产级 lint/test/build，也不进入反复 review/fix 循环。
- 用户确认后，删除原型或把结论沉淀到 `.codex/engineering-workflow/issues/`、`.codex/engineering-workflow/adr/`、design 或正式实现计划。

## 与 requirements-workflow 的关系

独立 UI 需求仍遵循 `requirements-workflow` 的 UI 原型门禁：默认生成可直接打开的静态 HTML 到 `.codex/engineering-workflow/issues/<REQ-or-EPIC>/prototype.html`。本 skill 补充更细的 UI/logic 原型执行方法。
