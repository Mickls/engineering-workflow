...
- 确认对应中文 review 辅助说明已创建或同步更新到当前工作产物根目录下的约定位置。
...
- 如果项目禁止 duplicate translated reports 或 bilingual protocol variants，确认中文辅助说明写在当前工作产物根目录，且没有污染生产 artifact 目录；这类工作产物 review aid 不视为 duplicate translated report，除非项目或用户明确禁止任何中文辅助文件。

## 需求覆盖核销

非轻量需求如果存在当前工作产物根目录 `issues/` 下的 issue/design/plan，声明完成前必须核销关键约束覆盖矩阵。非轻量 no-doc 例外必须核销临时计划或回复中的最小关键约束清单。
...
- 是否读取了当前工作产物根目录下的 `project/project-profile.md`、`commands.md`、`contracts.md`、`issue-workflow.md`、`context.md` 或 `context-map.md`。
...
- 创建、刷新或跳过的当前工作产物根目录下 `project/*` 文件。
...
- 运行期文件、缓存、构建产物、日志、会话登记表、fallback `.codex/engineering-workflow/` 中间产物、临时原型输出等如果已被 ignore，默认不提交，只在最终回复中说明其位置和用途。
