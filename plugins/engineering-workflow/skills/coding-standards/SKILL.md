...
- 阅读相关入口和周边实现。
- 读取当前工作产物根目录下的项目上下文：`project/project-profile.md`、`project/commands.md`、`project/contracts.md`、`project/issue-workflow.md`、`context.md` 或 `context-map.md`、相关 `adr/`。工作产物根目录由 `project-setup` 路由决定，Tolaria MCP 可用时优先使用 Tolaria project，失败时回退到 `.codex/engineering-workflow/`。
...
- 确认哪些输入已经在调用链顶层完成校验。
- 将修改范围限制在当前需求需要的行为上。
- 如果存在当前工作产物根目录 `issues/` 下的 issue/design/plan，先读取关键约束覆盖表和需求覆盖矩阵，不允许只凭总体描述开始编码。
...
1. 当前项目已有工具函数、通用模块、repository/DAO、service、client、组件或 helper。
2. 当前语言的标准库、运行时、框架原生能力或项目已采用的官方工具。
3. 项目已经引入的第三方库。
4. 自己实现。
...
如果当前工作产物根目录下的 `project/contracts.md` 已说明某类输入已标准化、非法值已拦截或依赖已保证存在，内部实现不得重复做同类防御。发现新入口不满足契约时，先更新契约或 design/plan，再决定是否补校验。
...
如果命中这些变化，交付前必须使用 `project-setup targeted-refresh` 更新工作产物根目录下对应的 `contracts.md`、`commands.md`、`project-profile.md`、`context.md` 或 ADR，并刷新 metadata。用户禁止写入当前工作产物根目录时，需要在最终回复中说明未回写的上下文和剩余风险。
