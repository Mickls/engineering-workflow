...
- 阅读相关入口和周边实现。
- 读取项目上下文：`docs/agents/project-profile.md`、`docs/agents/domain.md`、`CONTEXT.md`、相关 ADR；不存在时不阻塞，但不要凭空发明项目术语。
- 找到同项目中的相似代码。
- 识别已有 helper、service、repository/DAO、client、错误风格、日志风格和测试。
- 确认当前语言、框架、包管理器、lint/test/build 命令和生成代码流程。
- 确认哪些输入已经在调用链顶层完成校验。
- 将修改范围限制在当前需求需要的行为上。
- 如果存在 issue/design/plan，先读取关键约束覆盖表和需求覆盖矩阵，不允许只凭总体描述开始编码。

## 实现覆盖门禁

非轻量需求编码时，必须把实现和已确认 design/plan 的关键约束逐条对齐。

编码前：

- 从 `design.md` / `plan.md` 提取所有关键约束 ID。
...
- 在 `plan.md` 或对应 issue 记录中回填实际代码落点和状态。
...
- `nil` / `null` / `undefined` / 空请求。
- 用户输入字符串的 trim、normalize 和格式化。
- 枚举合法性。
- 必填 ID。
- 分页边界。
- 权限、租户范围和资源归属。
